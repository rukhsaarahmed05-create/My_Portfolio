...
function SkillsSection() {
    const { data: skills, isLoading } = (0, react_query_1.useQuery)({
        queryKey: ["/api/skills"],
    });
    ...
    if (isLoading) {
        return (<section className="py-16 bg-white dark:bg-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {Array.from({ length: 4 }).map((_, i) => (<card_1.Card key={i} className="animate-pulse">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {skills?.map((skill) => {
            ...
            return (<card_1.Card key={skill.id} className={`${colors.bg} border border-slate-200 dark:border-slate-700 hover:shadow-lg transition-shadow`}>
                <card_1.CardContent className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skill.technologies.slice(0, 2).map((tech) => (<badge_1.Badge key={tech} className={`${colors.badge} text-xs`}>
                        {tech}
                      </badge_1.Badge>))}
                  </div>
                </card_1.CardContent>
